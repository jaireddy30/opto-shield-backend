...
def init_db():
    ...
    cursor.execute("DELETE FROM events WHERE UPPER(sip_method) IN ('REGISTER', 'OPTIONS') OR UPPER(event) = 'SIP_AUTH' OR UPPER(caller_id) LIKE 'AGENT-%';")
    conn.commit()
    conn.close()
...
def db_get_historical_events(limit=500):
    ...
    try:
        ...
        cursor.execute("""
            SELECT * FROM events 
            WHERE UPPER(sip_method) NOT IN ('REGISTER', 'OPTIONS') AND UPPER(event) != 'SIP_AUTH' AND UPPER(caller_id) NOT LIKE 'AGENT-%'
...
def add_event(event):
    ...
    method = (event.get("sip_method") or "").upper()
    evt_type = (event.get("event") or "").upper()
    cid = str(event.get("caller_id") or "")

    # STRICT HARD FILTER: Drop REGISTER, OPTIONS, SIP_AUTH, and Agent-XXXX softphone pings from Call Logs
    is_softphone_ping = (method in ("REGISTER", "OPTIONS") or evt_type == "SIP_AUTH" or "AGENT-" in cid.upper())

    if not is_softphone_ping:
        db_save_event(event)
    ...
    # Emit ONLY real phone call events to live_event WebSockets UI stream
    if not is_softphone_ping:
        try:
...
# From & To headers for separating Caller ID and Destination Extension
FROM_RE        = re.compile(r"^From:\s*(?:\"[^\"]*\")?\s*<?sip:([^@;>\s]+)@", re.IGNORECASE)
TO_RE          = re.compile(r"^To:\s*(?:\"[^\"]*\")?\s*<?sip:([^@;>\s]+)@", re.IGNORECASE)
# X-Opto-Call-Id — stable call ID across retransmits (better dedup key than SIP Call-ID)
    ...
    with open(ASTERISK_LOG_PATH, "r", errors="ignore") as f:
        ...
        while True:
            ...
            if req_match:
                ...
                cur_method  = cur_callid = cur_cseq = cur_from_caller = cur_to_ext = cur_opto_id = None
                cur_ua      = "Opto-Web-Dialer"
                continue
            ...
            if mode == "request":
                ...
                if from_m:
                    cur_from_caller = from_m.group(1)
                    continue
                ...
                if c_match and cur_method in ("INVITE", "REGISTER", "OPTIONS", "BYE", "CANCEL"):
                    ...
                    if dedup_key not in done_ids:
                        ...
                        caller_val = _friendly_id(cur_from_caller or cur_to_ext)
                        ext_val = _friendly_id(cur_to_ext or cur_from_caller)
                        src_ip = ip_map.get(cur_to_ext, cur_ip) or ip_map.get(cur_from_caller, cur_ip)
                        ctx_val = determine_context(src_ip, "from-internal", caller_val, ext_val)
                        
                        # Session tracking for correlating BYE back to original numbers
                        if cur_method == "INVITE":
                            ...
                            if len(ACTIVE_CALL_SESSIONS) > 2000:
                                ACTIVE_CALL_SESSIONS.pop(next(iter(ACTIVE_CALL_SESSIONS)))

                        if cur_method in ("BYE", "CANCEL"):
                            saved = ACTIVE_CALL_SESSIONS.get(cur_callid) or ACTIVE_CALL_SESSIONS.get(cur_opto_id)
                            if saved:
                                if not caller_val or caller_val == "Unknown" or "Agent-" in str(caller_val) or caller_val.startswith("127."):
                                    caller_val = saved["caller"]
                                if not ext_val or ext_val == "-" or "Agent-" in str(ext_val) or ext_val.startswith("127."):
                                    ext_val = saved["extension"]
                                if src_ip in ("127.0.0.1", "10.0.0.1", None):
                                    src_ip = saved.get("source_ip", src_ip)
                                ctx_val = saved.get("context", ctx_val)
            ...
            if mode == "response":
                ...
                ext    = _friendly_id(info["extension"])
                caller = _friendly_id(info["caller"])
                # Skip REGISTER and OPTIONS softphone pings completely from call event stream
                if info["method"] in ("REGISTER", "OPTIONS"):
                    pending.pop(cur_callid, None)
                    continue

                event_type = "Cdr" if (info["method"] == "INVITE" and cur_status == "200") else "SIP_REQUEST"
            ...
            while True:
                ...
                while "\r\n\r\n" in buf:
                    ...
                    if fields.get("Event") == "Cdr":
                        ...
                        # Extract real caller ID number from AMI fields
                        ...
                        # Lookup real caller IP in cache before fallback
...
def tail_master_csv():
    target_path = ASTERISK_CDR_CSV_PATH
    if not os.path.exists(target_path):
        for alt in ["/var/log/asterisk/cdr-custom/Master.csv", "/var/log/asterisk/Master.csv"]:
            if os.path.exists(alt):
                target_path = alt
                break

    if not os.path.exists(target_path):
        print(f"  [!] Master.csv not found at {ASTERISK_CDR_CSV_PATH}")
        return

    print(f"  [+] Tailing {target_path} for completed CDR call records...")
    import csv
    seen_rows = set()

    # Pre-parse last 200 lines on startup
    try:
        with open(target_path, "r", errors="ignore") as f:
            lines = f.readlines()
            for line_str in lines[-200:]:
                line_str = line_str.strip()
                if line_str and line_str not in seen_rows:
                    seen_rows.add(line_str)
                    _parse_and_add_cdr_row(line_str, seen_rows)
    except Exception as e:
        print(f"  [!] CDR pre-parse warning: {e}")
    ...
    try:
        ...
        print(f"  [!] CDR CSV tailer error: {e}")

def _parse_and_add_cdr_row(line_str, seen_rows):
    import csv
    try:
        ...
        if len(parts) >= 12:
            src = parts[1].strip()
            dst = parts[2].strip()
            dcontext = parts[3].strip()
            clid = parts[4].strip()
            channel = parts[5].strip() if len(parts) > 5 else ""
            dstchannel = parts[6].strip() if len(parts) > 6 else ""
            lastdata = parts[8].strip() if len(parts) > 8 else ""
            start_time = parts[9].strip() if len(parts) > 9 and parts[9].strip() else datetime.now().isoformat()
            disposition = parts[14].upper() if len(parts) > 14 else "ANSWERED"
            billsec = float(parts[13]) if len(parts) > 13 and parts[13].replace('.', '', 1).isdigit() else 0.0

            # Clean Caller ID
            caller_id = _friendly_id(src) if src else "Unknown"
            if caller_id == "Unknown" or caller_id == "Opto Calling":
                caller_id = _friendly_id(clid)
            if caller_id == "Unknown" or caller_id == "Opto Calling":
                if channel:
                    caller_id = _friendly_id(channel.split('/')[1].split('-')[0] if '/' in channel else channel)
                else:
                    caller_id = "Opto Dialer"

            # Clean Extension / Destination
            extension = _friendly_id(dst)

            # Determine Context & Trunk Routing
            dst_lower = (dstchannel + " " + lastdata + " " + dcontext).lower()
            if any(k in dst_lower for k in ["plivo", "trunk", "pstn", "external", "inbound", "from-did"]):
                context = "from-external"
            elif "internal" in dcontext.lower() or "local" in dcontext.lower():
                context = "from-internal"
            else:
    try:
        f = open(target_path, "r", encoding="utf-8", errors="ignore")
        f.seek(0, 2)
        while True:
            line = f.readline()
            if not line:
            ...
            lastdata = parts[8]
            start_time = parts[9]
            answer_time = parts[10]
            end_time = parts[11]
            duration = int(parts[12]) if parts[12].isdigit() else 0
            billsec = int(parts[13]) if parts[13].isdigit() else 0
            disposition = parts[14] if len(parts) > 14 else "NO ANSWER"

            caller_id = src if src else clid
            extension = dst if dst else "-"
            context = dcontext if dcontext else "from-internal"

            source_ip = ENDPOINT_IP_CACHE.get(caller_id, ENDPOINT_IP_CACHE.get(extension, "127.0.0.1"))

            resp_map = {
                "ANSWERED": ("200", "16", "Normal Clearing"),
                "NO ANSWER": ("487", "19", "No Answer"),
                "BUSY": ("486", "17", "User Busy"),
                "FAILED": ("503", "38", "Service Unavailable"),
                "CONGESTION": ("503", "34", "Circuit/Channel Congestion")
            }
            resp_code, cause_code, cause_txt = resp_map.get(disposition, ("200", "16", disposition))

            formatted_ts = start_time.replace(" ", "T") if " " in start_time else start_time
